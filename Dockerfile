...
# Aufruf:   docker buildx build --tag juergenzimmermann/fussballverein:2023.1.1-jammy .
#           Get-Content Dockerfile | docker run --rm --interactive hadolint/hadolint:2.12.1-beta-debian
#           docker compose up
#           docker compose exec fussballverein bash
...
java -Djarmode=layertools -jar ./build/libs/fussballverein-2023.1.0.jar extract
EOF
...
RUN <<EOF
groupadd --gid 1000 fussballverein
useradd --uid 1000 --gid fussballverein --no-create-home fussballverein
EOF

COPY --from=builder --chown=fussballverein:fussballverein /source/dependencies/ ./
COPY --from=builder --chown=fussballverein:fussballverein /source/spring-boot-loader/ ./
#COPY --from=build --chown=fussballverein:fussballverein /source/snapshot-dependencies/ ./
COPY --from=builder --chown=fussballverein:fussballverein /source/application/ ./
